...
| DUP-2 | open | S | `src/autoupscale.c:438-470` — `EvenAlignSrcDims` and `EvenAlignSrcOffsets` are the same function modulo pointer type (same `up_chroma_to_zimg` probe, same `& ~1` on each subsampled axis). | The two call sites must stay in lockstep or crop dims and crop origin align differently — exactly the bug class each was added to fix. Fold together when fixing REL-1 (which changes the gate in both). |
...
| id | status | effort | description | notes |
|---|---|---|---|---|
| ARCH-1 | open | M | Chroma subsampling is encoded **twice** in production: the picture-view layout table (`src/picture_view.h:65-93`, `x_group_pixels`/`y_group_pixels`) and `up_chroma_to_zimg`'s `(sub_w, sub_h)` (`src/scaler_zimg_chroma.h:39-56`). | The two must agree (I420 group 2 ↔ shift 1) or plane extents and zimg graph geometry disagree silently on one chroma; nothing — no static assert, no shared table — ties them. One source of truth (derive the shifts from the layout table) removes the drift class *and* fixes REL-1 and DEC-1 for free. Distinct from the rejected test-side `min_plane_dims` item: this is production-to-production. |
...
| id | status | effort | description | notes |
|---|---|---|---|---|
| DEC-1 | open | S | The VLC entry TU includes `scaler_zimg_chroma.h` and calls `up_chroma_to_zimg()` purely to learn *generic* chroma subsampling for the crop even-align (`src/autoupscale.c:36,443,465`) — it runs even when swscale is the selected backend. | Breaks the documented layering (autoupscale → backend → helpers): the backend-agnostic entry TU takes a compile-time dependency on a zimg-specific header for a concept that is not zimg-specific — and that mis-gate is REL-1. Falls out for free once ARCH-1 lands. |
...
| id | status | effort | description | notes |
|---|---|---|---|---|
| REL-1 | open | M | `EvenAlignSrcDims`/`EvenAlignSrcOffsets` (`src/autoupscale.c:438-470`, called at `:575`, `:766`) gate on `up_chroma_to_zimg()`, which returns 0 for **NV12/NV21** — both 4:2:0, subsampled on both axes and routed to swscale. So the two chromas every hardware decoder emits get neither even-aligned crop dims nor even-aligned crop offsets. | Found independently by two auditors. NV12 clip with `i_x_offset == 1`: `up_picture_plane_extent` floors the chroma anchor to `x/2 = 0` while luma starts at byte 1 — chroma displaced half a luma pel for the whole playback, i.e. exactly the colour-fringing bug this code was added to fix for I420/YV12/I422/I444. Odd `i_y_offset` does the same vertically; odd `src_w/src_h` also reach `sws_getContext` for a 4:2:0 pix_fmt. Not memory-unsafe (extent checks still bound the read), purely a registration error, and invisible to the byte-identity tests because both backends share `picture_view`. Fix: gate on the *chroma's* subsampling via the `picture_view` layout table (see ARCH-1), not on zimg support. |
| REL-2 | open | S | `up_decide_tile_grid` (`src/plane_utils.h:126-152`) bounds the row count only by `dst_h / stripe_min` and never looks at `src_h`, so the corresponding **source** stripe can be empty; `up_compute_stripe_bounds` then returns 0 and kills the whole zimg backend (all-or-nothing construct). | Reproduced numerically: src 320×100 → dst 1280×400, `--autoupscale-threads=64 --autoupscale-zimg-stripe-lines=4` → 64×1 grid; cell 0 gets dst `[0,6)` but src `[0,0)` → `try_spawn_one_worker(-1)` → teardown → sticky `lazy.failed` → backend dead from frame 1 (AUTO silently degrades to swscale; `--autoupscale-backend=1` drops 100% of frames). 14 of 64 cells are degenerate there. Reachable for any `zimg-stripe-lines` in 1..7 — a range the option's own longtext encourages. The comment at `src/scaler_zimg.c:907-913` ("the grid … makes every cell ≥ stripe_min rows … so geometry never degenerates") is false. |
