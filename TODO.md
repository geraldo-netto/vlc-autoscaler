...
|---|---|---|---|---|
| UB-1 | open | S | `src/perfmon.h:99-100` right-shifts a possibly-negative `int64_t` (`diff >> UP_PERFMON_ALPHA_SHIFT`); implementation-defined per C11 6.5.7p5, and the comment at perfmon.h:94-98 wrongly claims C11/C18 define it as arithmetic shift. | Behavior fine on gcc/clang; either use `diff / 8` (fully portable, same codegen) or fix the comment to declare a deliberate IDB dependency. Cross-confirmed by two independent audits. Also a portability item. |
...
| PORT-3 | open | S | `Makefile:64` — `MARCH ?= native` feeds `-march=$(MARCH)` everywhere; nothing stops `make MULTIVERSION=1` on non-x86 until the compiler dies deep in the build (`usm_pool_dispatch.c:54-56` `#error`s correctly). | Self-defending at runtime (PORT-6 level gates); an early Makefile arch check would fail fast with a clear message. |
...
| id | status | effort | description | notes |
|---|---|---|---|---|
| BUILD-1 | open | M | Missing header dependencies in all non-`-MMD` explicit Makefile rules — empirically verified: after `touch src/threading.h`, `make -q build/test_usm_pool` says up-to-date (usm_pool.c includes it); same for `src/upscale_logic.h` vs test_scaler_swscale. Same class: rules listing `src/usm.h` miss `src/zimg_helpers.h` (usm.h:29); `ZIMG_H_DEPS` (Makefile:468-473) misses `src/chroma_classify.h` (via scaler_zimg_chroma.h:27). | Only plugin objects get `-MMD -MP`. Adding `-MMD` to TEST/SMOKE/FUZZ/STRESS/COV compile rules fixes the whole class; stale-object risk is real for incremental local dev (CI builds clean, so it hides there). |
| BUILD-2 | open | S | `Makefile:165-175` vs `:187-188` — the friendly "vlc-plugin pkg-config not found" diagnostic lives in the `.so` link recipe, but object compilation runs first with empty `VLC_CFLAGS`, dying on cryptic `vlc_common.h: No such file` before the message is reached. | Reproduced on this host. Move the check to a compile-order prerequisite (or `$(error)` at parse time when the target needs it). |
| BUILD-3 | open | S | CI never builds `make plugin MULTIVERSION=1` (.github/workflows/ci.yml:14-19,42-54): the production dispatcher configuration — PLUGIN_CFLAGS + `-O3 -flto` + `.so` link resolving the un-suffixed API against usm_pool_dispatch.o — is exactly the unresolved-symbol hazard usm_pool.h:22-39 warns about, and it is unexercised. | Symbol wiring verified consistent today (scratch build + `nm`), but CI wouldn't catch regressions. Add one CI build step. |
| BUILD-4 | open | S | The sonarcloud CI job (.github/workflows/ci.yml:143-149) omits `libzimg-dev`, so `scaler_zimg.c` (largest TU, 1350 lines) is never compiled under build-wrapper; `make analyze` cppcheck (Makefile:742-745) also excludes it — the file gets no static analysis at all in CI. | Add libzimg-dev to the sonar job deps; consider including it in cppcheck. |
| BUILD-5 | open | S | `USM_POOL_FLAT_SKIP=1` compile path (src/usm_pool.c:175-210, Makefile:565-570) is built only by `build-bench`/`bench-flatskip`, which no CI step invokes — the flat-skip code can rot without any compile check. | Cheapest fix: add `build-bench` to a CI job (compile-only). |
| BUILD-6 | open | S | `Makefile:191` — `test:` hard-depends on `complexity` (lizard), so `make test` errors on machines without lizard even though tests don't need it. | Split: keep `check`/CI running both, make `test` run tests only, or degrade gracefully when lizard is absent. |
