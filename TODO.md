...
| PORT-11 | open | S | `tests/test_usm_pool_dispatch.c:20-32` creates an opaque sentinel with `(usm_pool_t *)(uintptr_t)0x1`. | Integer-to-pointer conversion is implementation-defined and unnecessary here. Return an address-backed static object/sentinel through the opaque pointer type. |
...
| BUILD-22 | open | S | The mandatory coverage target is red on HEAD: `up_chroma_align_axis_even` is 88.9%, below the 90% per-function threshold, because `src/chroma_classify.h:265-267` is uncovered when both pointers are present and `*dim <= 0`. | Fresh `make coverage ... EXTRA_CFLAGS=-Werror` exits 2 despite 96.9% file coverage. Add zero and negative full-axis cases, including offset preservation, and restore the gate. |
...
| OBS-11 | open | S | Regular coverage runs every binary unlabeled with stdout/stderr redirected to `/dev/null` (`Makefile:915-917`). | On a crash the target reports only the loop's generic Make error, hiding the failing binary and sanitizer diagnostics. Print each binary and preserve/replay captured output on failure. |
