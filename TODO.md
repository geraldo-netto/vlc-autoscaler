...
| PERF-14 | blocked | M | Validate resolution-based static USM AUTO presets before changing defaults. | The [fixed-grid sweep](docs/PROFILING.md#frame-latency-and-worker-count) finds the lowest sampled mean at 8/12/16 USM workers for 720p/1080p/4K output on the Ryzen 9 7945HX, with zimg held at 12. Eight was the 720p result; at 1080p eight takes 266.9 us/frame versus 258.9 at twelve. Applying these counts through the existing shared `threads` preference (`src/autoupscale_module.c`, `InitUsmPool` in `src/autoupscale.c`, and `src/scaler_zimg.c`) would also change zimg's graph grid, conflicting with unchanged-pixel acceptance. A USM-only AUTO policy avoids that coupling. Unblock by validating the proposed counts against current AUTO on representative paced content and different CPU budgets, preserving explicit preferences, graph geometry and pixels. The 720p mean gain is only 1% with mixed tail results; 4K sixteen improves mean by 8% but uses about 12% more CPU and exceeds the current twelve-worker AUTO ceiling. Define CPU headroom and intermediate/atypical-resolution behavior before selecting thresholds; the measured presets are candidates, not proven universal defaults. |
