...
| OBS-2 | open | S | `filter_NewPicture` exhaustion (`autoupscale.c:1067`) does `dropped_count++; picture_Release; return NULL` with no log at any level; under output-pool backpressure every frame drops silently. | Add a one-shot `msg_Warn` mirroring the `process_fail_logged` latch pattern. |
...
| OBS-4 | open | S | `dropped_count` is not exported as a VLC variable (`MaybeLogStats` exports `ewma-us` + `frames` only, `autoupscale.c:981`), though it is the most operationally interesting counter for "why is playback stuttering." It appears only in the throttled 5 s `msg_Dbg` line. | Export `autoupscale-dropped` alongside the existing pair. |
