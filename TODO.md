...
| UNUSED-3 | open | S | Ten `static inline` functions have no production-rooted caller: the USM reference subtree (`src/usm.h:66-74,175-186,237-255,273-308`), the content reference subtree (`src/content_probe.h:71-111,134-193`), and `up_worker_pool_inline` (`src/worker_pool.h:154-157`). | Move intentional reference helpers and support types to a clearly named support header; delete the unconsumed getter. Neutral worker wording avoids widening the opaque USM API. |
