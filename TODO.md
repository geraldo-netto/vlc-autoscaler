...
|----|--------|-------------|------------|
| SCAL-3 | L | 2D/column tiling for very wide/short zimg frames | BLOCKED on a zimg-API approach. Seam oracle + fuzzer landed (commit 57bbf7f): `test_tiling_matches_untiled` (smooth, maxdelta<=6) + `fuzz_scaler_seam` (randomized geometry, maxdelta<=16, libFuzzer+smoke in `make test-zimg`). They first established that the existing N-ROW-stripe scaler is not byte-identical to a single-graph resample — sub-pixel PHASE rounding at boundaries, <=3/255 on real content (imperceptible), no visible bug. KEY FINDING (2026-05-30): a NAIVE column tile (offset the plane pointer + shrink the declared width) is MEMORY-UNSAFE with zimg — `zimg_filter_graph_process` over-reads/over-writes past the declared tile width (harmless at full width, lands in row padding; for a sub-width column it crosses the column boundary / buffer end). The seam fuzzer caught this instantly as an ASan SEGV inside libzimg (e.g. 358x56->372x102 YV12, 6x2 grid). zimg does NOT support processing a column sub-rectangle via pointer offset. Correct column tiling needs zimg's `active_region` (subpixel source window, zimg.h:571-575) on FULL-WIDTH buffers, with the dst written per-tile — a larger, uncertain effort (dst sub-width SIMD tail behavior needs verifying). Row striping already covers all realistic frames; column tiling only helps very wide+short frames. Re-scoped S->L, parked behind the now-existing oracle+fuzz which any correct attempt must pass. |
