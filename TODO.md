...
| ARCH-5 | open | S | `usm_pool.h:8-15` documents the REMOVED two-phase algorithm ("two-phase dispatch with a barrier between phases … Phase 1 … Phase 2 …") as the pool's design; `usm_pool.c:14-28` says the fused single-pass sweep replaced it ("Earlier versions ran two dispatch phases"). Header-vs-impl doc drift in the public API header. | Fix: replace the stale paragraph with one line describing the fused single-pass sweep (or point at usm_pool.c's note). A reader of the header alone currently learns an algorithm, barrier, and shared workspace that no longer exist — strictly fewer wrong concepts after the fix. |
